...
export const SEED_CONNECTORS: LocalConnectorRegistryItem[] = [
  {
    ...
    health_note: 'Health check via gdrive-read Edge Function — read-only proxy, no writes.',
    safety_note: 'Sandbox mode — health check via Edge Function proxy only. No webhook trigger, no workflow run, no data written. Requires owner approval before any production use.',
    created_at: NOW,
    updated_at: NOW,
  },
  {
  ...
  },
  {
    ...
    status: 'configured',
    mode: 'sandbox',
    description: 'Google Drive API for asset storage, export pack delivery, and document management. Phase 1 — health-only via Edge Function, no real Google API call yet.',
    required_env_keys: ['GDRIVE_SERVICE_ACCOUNT_JSON', 'GDRIVE_FOLDER_ID'],
    last_checked_at: null,
    health_note: 'Health check via gdrive-read Edge Function — Phase 1, no Google API call.',
    safety_note: 'Read-only sandbox. No write, no delete, no publish. Service account key lives in Supabase vault only — never in frontend or repo. Phase 1 — health ping only.',
    created_at: NOW,
    updated_at: NOW,
  },
  {
...
export const SEED_EVENTS: LocalModuleEvent[] = [
  ...
  },
  {
    ...
    direction:    'inbound',
    status:       'needs_review',
    related_client_id:    'client-moc-an',
    related_brand_id:     'brand-moc-an',
    related_campaign_id:  'campaign-moc-an-s1',
    related_content_item_id: null,
    payload_preview: '{"report_type":"campaign_progress","scope":"Cold Brew S1 Launch Campaign","period":"2026-06","items_approved":4,"items_pending":3,"source":"mock"}',
    error_message: null,
    received_at:  PAST(30),
    processed_at: null,
    created_at:   PAST(30),
