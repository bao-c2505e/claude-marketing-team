# CURRENT PHASE — 🏁 CORE MVP CLOSED — 18/18 PHASES COMPLETE (2026-06-11) | Ver2: V2-UI-T1 Manual E2E + UI Bug Triage ✅ DONE / PASS (Create Brand success-feedback fix + mobile responsive polish — 2026-06-15; separate from V2-E3) | Ver2: V2-E3 PC2 Adapter Skeleton 🔴 NOT STARTED / Owner-gated (checkpoint O1) | Ver2: V2-D2 Supabase Staging Execution 🟡 PARTIAL / BLOCKED ON STAGING VERIFICATION (Checkpoint B blocked = the staging-verification blocker; C/D/E are independent docs/spec/policy/planning that do NOT complete or replace staging verification; overall V2-D2 NOT fully DONE/PASS until B unblocked + verified) — Checkpoint A ✅ PASS (process/docs) / Checkpoint B 🔴 VERDICT = BLOCKED (no disposable staging env — 2026-06-15; B2 unblock attempt 2026-06-16 = STILL BLOCKED, env all MISSING — see staging report §13) / Checkpoint C ✅ DOCS/SPEC DONE (client-feedback policy — 2026-06-15) / Checkpoint D ✅ DONE — Owner decision recorded (A/B/C/D = YES; B metadata-only — 2026-06-15, NOT implemented) / Checkpoint E ✅ DOCS/SPEC DONE — feedback implementation PLAN only (2026-06-15, NOT implemented) / Checkpoint F (migration draft) 🔴 NOT STARTED Owner-gated | Ver2: V2-D1.5 Manual E2E Checklist + Demo Script ✅ DONE (docs-only — 2026-06-12) | Ver2: V2-E2 Core ↔ PC2 Dry-run Integration Plan ✅ DONE (2026-06-12) — V2-E3 adapter skeleton 🔴 NOT STARTED, Owner-gated (checkpoint O1) | Ver2: V2-E1 Core ↔ PC2 Contract Mapping Spec ✅ DONE / PASS (2026-06-12) | Ver2: V2-D1 Supabase Staging Audit & Runbook ✅ DONE (2026-06-12) — V2-D2 staging execution 🔴 NOT STARTED, Owner-gated | Ver2: V2-C Client Demo Package ✅ DONE / PASS (Owner-rehearsed — 2026-06-12) | Ver2: V2-A Manual Browser E2E + Demo Script ✅ DONE / PASS (Owner-executed — 2026-06-12) | Ver2: V2-B UI Polish (Owner-directed) ✅ DONE (2026-06-11) | Ver2 roadmap ✅ DONE (2026-06-11) | Phase 18 ✅ CLOSED (Codex PASS — 2026-06-11) | Phase 17 ✅ CLOSED (Codex PASS — 2026-06-11) | Phase 16D ✅ CLOSED (Codex PASS — 2026-06-11) | Phase 16C-2 ✅ CLOSED (Codex PASS — 2026-06-11) | Phase 16C-1 ✅ CLOSED (Codex PASS — 2026-06-11) | Phase 16B-2 ✅ CLOSED (Codex PASS — 2026-06-10) | Phase 16B-1 ✅ CLOSED (Codex PASS — 2026-06-10) | Phase 16A ✅ CLOSED (Codex PASS — 2026-06-09)

## 📌 Thông tin chung
- **Mới nhất:** **Phase C — Manual n8n Prompt Update Pack / Owner Activation Pass** ✅ **DONE / PASS** (2026-06-18) — docs/runbook only. Owner activation pack để **thủ công** cập nhật 5 production n8n OpenAI prompt bằng chất lượng FnB đã tune ở **Phase B** (AI Factory V1 FnB Output Quality Tuning ✅ PASS). Deliverables: `07_runbooks/phase_c_manual_n8n_prompt_update_owner_activation_pass.md` (Sections A–G + 5 khối "COPY THIS PROMPT" verbatim từ runbook Phase B — KHÔNG bịa prompt mới) + `08_logs/phase_c_manual_n8n_prompt_update_pack_20260618.md`. **Per-module:** Content Factory V1 = production-active (OpenAI node thật) → paste trực tiếp, expect 7-day pack (7 items); Design/Video/Ads/Report = Owner-gated, production chạy Local fallback + placeholder → prompt dùng khi Owner kích hoạt Path B, mỗi module expect đúng 5 items. **Không cần** re-import workflow JSON / Core redeploy / Vercel env change cho prompt update; OpenAI key chỉ ở n8n Credentials. **Safety:** approval-first; Approved ≠ Published; no auto-post/auto-ads/schedule/spend/publish; no live connectors; no image/video generation; no live analytics pull; no fake metrics; no secrets/webhook URL committed; no workflow JSON changed. Build PASS (no >500 kB warning), test 75/75 PASS, contract validation ALL PASS. `3656059` (Phase B rollup) đã push `origin/main` từ trước; Phase C commit thêm lên trên + push, no history rewrite.
...
- **Mới nhất:** **V2-E3 PC2 Adapter Skeleton — START GATED 🔴 (KHÔNG start)** (2026-06-16): Owner direction = start V2-E3 chỉ nếu prerequisite gate pass. **Gate FAILS** → KHÔNG start (no adapter code/tests/spec). Lý do: B2 = 🔴 BLOCKED (staging report §13.10/§14; env re-confirm tất cả MISSING); overall V2-D2 = 🟡 PARTIAL/BLOCKED (không VERIFIED); tracking KHÔNG explicitly cho phép V2-E3 parallel; checkpoint O1 chưa logged. Hard rule "if B2 BLOCKED → do NOT start V2-E3, stop" đã fire. Architecturally V2-E3 (pure local builder, zero HTTP, kill-switch) độc lập với Supabase staging — Owner có thể unblock bằng (A) provision disposable staging → re-run §14.1 → B2 VERIFIED; **hoặc** (B) ghi rõ trong tracking V2-E3 được phép chạy song song (staging blocker = non-blocking cho adapter) **VÀ** log O1. **V2-E3 + real connector activation + feedback implementation + Checkpoint F vẫn 🔴 NOT STARTED / Owner-gated.** No approval mutation; PC2 callbacks metadata/log/echo only. Docs/logs only; build PASS, 45/45 tests.
- **Trước đó:** **V2-D2 — B2 verdict routing → Route B** (2026-06-16): Xác định verdict B2 thực tế từ staging report (§13.10 + §14) = 🔴 **BLOCKED** (classify đúng một trong VERIFIED/PARTIAL/BLOCKED/FAILED). **"Codex PASS" cho B2 = report được review TRUTHFUL, KHÔNG phải staging VERIFIED.** Re-confirm env presence-only (không in giá trị): tất cả staging vars MISSING, `.env*` ABSENT. → **Route B chọn:** KHÔNG proceed UI/product implementation; **Route A (V2-UI-T2 Manual Product E2E) KHÔNG start** (dựa trên B2=VERIFIED, chưa đạt). Report §14 + §14.1 (exact re-run command/check một khi disposable staging sẵn sàng). **Overall V2-D2 vẫn 🟡 PARTIAL / BLOCKED ON STAGING VERIFICATION (KHÔNG DONE/PASS).** V2-UI-T1 ✅ DONE/PASS (separate from V2-E3). **V2-E3 PC2 Adapter Skeleton + feedback implementation + Checkpoint F migration draft vẫn 🔴 NOT STARTED / Owner-gated — không cái nào start.** Docs/logs only; build PASS, 45/45 tests.
- **Trước đó:** **V2-D2 — Checkpoint B2 — Unblock Attempt** 🔴 **STILL BLOCKED** (2026-06-16): Owner direction = unblock disposable Supabase staging verification. Env presence check (redacted, KHÔNG in giá trị): `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY`/`SUPABASE_ACCESS_TOKEN`/`SUPABASE_PROJECT_REF`/`DATABASE_URL`/service-role = **tất cả MISSING**; `.env*` ABSENT; no linked `supabase/` project; CLI not installed. → **Verification KHÔNG chạy** (no disposable staging); halt trước mọi DB action, KHÔNG SQL, KHÔNG connect, KHÔNG production, KHÔNG fake. Migrations/schema/RLS-cross-tenant = NOT EXECUTED; approval/callback safety = design-verified (no runtime callback path) nhưng KHÔNG thay thế DB-level RLS. **Verdict B2 = 🔴 BLOCKED.** Report `08_logs/v2_d2_staging_report_20260615.md` §13. **Checkpoint B vẫn 🔴 BLOCKED; overall V2-D2 vẫn 🟡 PARTIAL / BLOCKED ON STAGING VERIFICATION.** Next: Owner tạo `core-agency-staging-disposable` + 4 fictional users + `.env.local` staging anon URL/key (no service-role frontend) → PC1 drive M1–M10 + §6/§7 → re-issue B verdict. **C/D/E vẫn ✅ docs-spec DONE độc lập; V2-E3 PC2 Adapter Skeleton + feedback impl + Checkpoint F vẫn 🔴 NOT STARTED / Owner-gated.**
- **Trước đó:** **V2-UI-T1 — Manual E2E + UI Bug Triage** ✅ **DONE / PASS** (2026-06-15): Owner pause feedback implementation → triage 2 issue. **(Disambiguation: đây là V2-UI-T1, một checkpoint UI/manual smoke-test triage RIÊNG; KHÔNG phải V2-E3. V2-E3 = PC2 Adapter Skeleton vẫn 🔴 NOT STARTED / Owner-gated — checkpoint O1. V2-UI-T1 DONE KHÔNG bắt đầu/hoàn thành V2-E3.)** **Issue Create Brand:** root cause KHÔNG phải broken handler — local create flow đúng; defect = **UX gap (success path im lặng)** → fix = visible auto-dismiss success banner trong `BrandsTab.tsx`. **Issue mobile rối:** root cause = `index.css` không có `@media` + main grid inline `260px 1fr` không collapse → fix = `app-main-grid`/`app-sidebar` classes (App.tsx) + responsive block (≤768px/≤480px) trong `index.css`; **desktop unchanged, no redesign**. Latent UUID-gating gap (brands/clients thiếu per-op Supabase-vs-local fallback) document làm follow-up, KHÔNG fix. Manual E2E smoke (client→brand→campaign→brief) OK; approval an toàn; PC2 callbacks non-authoritative. **Safety:** no Supabase/RLS/auth/migration/SQL/secrets/connectors; no feedback implementation; approval semantics unchanged; no connector activation; no real posting/ads/messaging/customer contact; approval state remains Core UI-authoritative. Files: `BrandsTab.tsx` + `App.tsx` + `index.css` + `08_logs/v2_ui_t1_ui_bug_triage.md`. Build PASS, 45/45 tests. **Feedback implementation (Checkpoint F) vẫn 🔴 NOT STARTED / Owner-gated.**
- **Trước đó:** **V2-D2 — Checkpoint E — Feedback Implementation Plan** 🟡 **PLAN ONLY** (2026-06-15): Owner duyệt bắt đầu Checkpoint E = feedback implementation **planning** (docs/spec/implementation-plan only). Hai deliverable mới: `03_core/specs/v2_d2_feedback_implementation_plan.md` (purpose/scope/non-goals/accepted-policy summary/data model `client_feedback`/RLS design/repository-API/UI/audit-logging/migration outline/test plan/rollout/risks/open questions/next checkpoint) + `03_core/specs/v2_d2_feedback_future_checkpoints.md` (Checkpoints F–J, mỗi cái scope/allowed/hard-boundaries/validation/Codex-expectations; tất cả NOT STARTED/Owner-gated). **Invariants giữ nguyên:** client/viewer feedback KHÔNG mutate `approval_status`; **viewer strictly read-only** (loại khỏi mọi write predicate); approver feedback/request-revision OK, approved-like = metadata only cần Owner/Internal confirm; PC2 callbacks non-authoritative; KHÔNG feedback/callback-driven posting/ads/messaging/customer contact. **No implementation** (no code/migrations/RLS/SQL/tests/secrets/connectors; no staging/production). **Checkpoint F (migration draft) 🔴 NOT STARTED / Owner-gated** (build checkpoints G+ còn cần Checkpoint B *VERIFIED* — hiện BLOCKED).
- **Trước đó:** **V2-D2 — Checkpoint D — Owner Decision** ✅ **DONE** (2026-06-15): Record Owner decision và **đóng policy-decision stage** (Checkpoints C → D), documentation/log/decision-record only. **Owner decision:** **A = YES** (client approver feedback/request-revision, no state mutation) · **B = YES, metadata only** (approved-like feedback cần Core owner/internal confirmation trước mọi real approval state change — không auto-approve) · **C = YES** (viewer read-only) · **D = YES** (separate feedback table, future). **Decision status: ACCEPTED / OWNER-APPROVED FOR FUTURE IMPLEMENTATION** — phủ policy *direction*, KHÔNG authorize build bây giờ. **Invariants preserved:** client feedback KHÔNG mutate `approval_status`; approved-like = metadata only; rejected/needs_revision-like = metadata/feedback-record; viewer read-only; PC2 callbacks = metadata/log/echo (non-authoritative); KHÔNG feedback/callback-driven posting/ads/messaging/customer contact. Updated: `03_core/specs/v2_d2_checkpoint_c_decision_record.md` (+§7 Owner Decision, +§8 Status) + `v2_d2_client_feedback_policy.md` (wording §11 → approved A–D). **No implementation.** **Future implementation phase 🔴 NOT STARTED / Owner-gated** (theo sau Checkpoint B *VERIFIED* — hiện BLOCKED).
...
> **Overall V2-D2 status: 🟡 PARTIAL / BLOCKED ON STAGING VERIFICATION.** V2-D2 is **NOT
> fully DONE/PASS** and must not be claimed as such until Checkpoint B is unblocked and
> verified. **Checkpoint B is the staging-verification blocker.** Checkpoints C/D/E are
> docs/spec/policy/planning work completed **independently** — they **do not complete or
> replace** the staging verification, and their DONE status does not make V2-D2 done.
>
> **Checkpoint A:** ✅ PASS (process/docs — Codex-reviewed). **Checkpoint B verdict:** 🔴 BLOCKED — DB-level verification could not run (no disposable staging env). **Checkpoint C:** ✅ DOCS/SPEC DONE (client-feedback policy). **Checkpoint D:** ✅ DONE (Owner decision docs). **Checkpoint E:** ✅ DOCS/SPEC DONE (implementation PLAN only). **Checkpoint F (migration draft):** 🔴 NOT STARTED / Owner-gated.
...
### Checkpoint E — Feedback Implementation Plan (🟡 PLAN ONLY — 2026-06-15):
- **Kết quả:** Owner duyệt Checkpoint E = feedback implementation **planning** (docs/spec/implementation-plan only). **Status: 🟡 PLAN ONLY / NOT implemented.** Feedback implementation 🔴 **NOT STARTED**.
- **Nội dung plan:** separate `client_feedback` table (fields + tenant/scope + parent refs + actor identity + feedback_type/feedback_status + immutable audit); RLS design (scoped reads only; **client approver INSERT only trong tenant**; **viewer read-only — không trong write predicate**; inactive/expired denied; read/write role separation; parent hierarchy validation; no broad OR-scope bypass; no callback impersonation); repository/API (interfaces, safe create/list, **no client update/delete**, UUID-gating); UI (feedback panel, "Client feedback" label, approval controls owner/internal-only tách riêng, viewer read-only); audit/logging; migration outline (future-only, NOT executable, staging-first); test plan; rollout; risks R1–R8; open questions.
- **Preserved invariants:** client/viewer feedback KHÔNG mutate `approval_status`; viewer strictly read-only; approver approved-like = metadata only cần Owner/Internal confirm; PC2 callbacks non-authoritative; KHÔNG feedback/callback-driven posting/ads/messaging/customer contact.
- **Files (Checkpoint E):**
  | File | Change |
  |---|---|
  | `CLAUDE_MARKETING_TEAM/03_core/specs/v2_d2_feedback_implementation_plan.md` | NEW — implementation plan |
  | `CLAUDE_MARKETING_TEAM/03_core/specs/v2_d2_feedback_future_checkpoints.md` | NEW — Checkpoints F–J outline |
  | `CURRENT_PHASE.md` / `SESSION_SUMMARY.md` / `08_logs/phase_log.md` / `08_logs/agent_activity_log.md` | Checkpoint E log |
- **Future checkpoints:** F (migration draft) / G (staging RLS verify) / H (repository-API) / I (UI) / J (manual E2E + evidence) — tất cả 🔴 **NOT STARTED / Owner-gated**; G+ còn cần Checkpoint B *VERIFIED* (hiện BLOCKED).
...
3. Checkpoint B (staging verification verdict) is the blocker → currently 🔴 BLOCKED, so **overall V2-D2 remains 🟡 PARTIAL / BLOCKED ON STAGING VERIFICATION** (C/D/E docs/spec done independently do NOT replace staging verification). V2-D2 closes as DONE only when B is unblocked and verified. *(Note: this older plan listed only B+C; the model later expanded to Checkpoints A–F.)*
