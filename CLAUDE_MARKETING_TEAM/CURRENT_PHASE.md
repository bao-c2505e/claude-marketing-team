...
- **Mới nhất:** **V2-E2 (Owner naming) — Core ↔ PC2 Dry-run Integration Plan** ✅ DONE (2026-06-12) — planning/contract/test-design only, xem section bên dưới. **Scope ladder được Owner refine: V2-E2 = PLAN; implementation tách thành V2-E3 (adapter skeleton, disabled by default) → V2-E4 (local-only callback ingest test) → V2-E5 (PC2 dry-run E2E, mock endpoint only) → V2-E6 (closure/evidence pack) — mỗi phase Owner-gated qua checkpoints O1–O5.** V2-E3 🔴 NOT STARTED — checkpoint O1 (env var/visibility flag) bắt buộc trước khi bắt đầu.
