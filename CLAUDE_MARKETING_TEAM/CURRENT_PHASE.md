...
- **Mới nhất:** **V2-E1 (Owner naming) — Core ↔ PC2 Contract Mapping Spec** ✅ DONE (2026-06-12) — documentation/contract mapping only, xem section bên dưới. **V2-E2 (dry-run implementation) 🔴 NOT STARTED — cần Owner approval TRƯỚC khi implement bất kỳ runtime integration nào.** *(Naming note: trong roadmap, PC2 n8n dry-run vốn là roadmap-V2-C còn "V2-E" là UI polish — theo naming của Owner: V2-E1 = mapping spec, V2-E2 = dry-run implementation.)*
...
- **Tiếp theo:** Owner đọc mapping spec → nếu duyệt V2-E2, implement dry-run only theo spec (local mock, kill-switch, no secrets). Song song: Owner xem V2-D runbook → checkpoint A → V2-D2. Các package khác còn mở: UI polish/code-splitting (roadmap-V2-E), V2-F connector plan (Owner-gated).
