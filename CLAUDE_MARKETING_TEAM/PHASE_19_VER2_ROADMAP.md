# VER2 ROADMAP — POST-MVP PLANNING + CONTROLLED DEMO ROADMAP

**Date:** 2026-06-11
**Baseline:** 🏁 **Core MVP CLOSED — completed in 18/18 phases** (closure commit `e3f1ed7`, closure report: `CORE_MVP_CLOSURE_REPORT.md`)
**Nature of this document:** documentation/planning ONLY — no product code, no connectors, no secrets, no live automation.

> **Naming clarification:** The original Core MVP was planned and delivered as **18 phases (Phase 1–18)**. Phase 18 was the final Core MVP phase, and the Core MVP is **closed**. This document is the **Post-MVP / Ver2 Planning workstream** — it does NOT add more MVP phases, and the work below is not "MVP Phase 19". (The filename keeps its historical `PHASE_19_` prefix only to preserve existing links; the workstream itself is Ver2.)

This document is the Ver2 planning package: scope options, recommended priority order, and a breakdown into small, individually reviewable Ver2 work packages (**V2-A … V2-F**), each with goal, deliverables, files touched, safety rules, checks, Codex review focus, and an Owner approval checkpoint.
...
| 3 | Supabase staging hardening | Apply migrations to a **staging** project, create real role rows, verify every RLS policy per role, resolve client-feedback policy | 🟡 Medium — first time SQL touches a live (staging) DB | Option 1 (verified app) |
...
**Do first (now):**
1. **V2-A — Manual E2E pass + demo script** (option 1 + start of 2). Zero risk, unblocks everything: it is the only remaining verification gap in the closed MVP, and both the demo and the Supabase work assume a verified UI.

**Do second:**
2. **V2-B — Supabase staging hardening** (option 3). The highest-value technical step: every later option needs a real database with verified RLS. Staging project only — production env stays OFF.
3. **V2-D — Client demo package** (option 2) can run **in parallel** with V2-B — it is content/docs work on a different file set.

**Can wait / parallel when idle:**
4. **V2-E — UI/brand polish** (option 5). Valuable but never urgent; safe to schedule into gaps. Code-splitting before any public-facing deployment.

**Do last + gated (needs Owner approval to even start):**
5. **V2-C — PC2 n8n/modules dry-run** (option 4). First time an external process talks to Core; requires a backend for `WEBHOOK_SHARED_SECRET` (never frontend) and the staging DB from V2-B.
6. **V2-F — Real connector readiness plan** (option 6). Planning doc first; any actual activation is a separate, individually Owner-approved work package per connector. **No connector goes live inside Ver2 without a written Owner sign-off per connector.**

**Risky items requiring explicit Owner approval before starting:**
- Applying SQL to ANY live database (V2-B — staging counts).
- Running n8n against Core, even dry-run (V2-C).
- Any connector activation, auto-post, ads, or messaging (V2-F — and these stay OFF for all of Ver2 planning).
- Putting real client data anywhere (not in Ver2 scope at all until RLS is verified in V2-B and the Owner approves).

## 3. Ver2 work-package breakdown

### V2-A — Manual Browser E2E Pass + Demo Script Verification
...
- **Deliverables:**
  ...
  - `08_logs/ver2_a_e2e_results.md` — run log: environment, browser, date, defects found.
...
- **Files likely touched:** `08_logs/phase_17_e2e_checklist.md`, new `08_logs/ver2_a_e2e_results.md`, `07_docs/client_demo_script.md`; product code ONLY for triaged blocker fixes (each its own commit).
...
- **Owner checkpoint:** Owner reviews results doc and approves "demo-able" status. **Gate to V2-B/V2-D.**

### V2-B — Supabase Staging Hardening
...
- **Deliverables:**
  ...
  - RLS verification matrix: new `08_logs/ver2_b_rls_verification.md` — per table × per role × per operation (SELECT/INSERT/UPDATE), expected vs actual, incl. the known client-feedback case.
...
- **Files likely touched:** `03_core/database/` (new MIGRATION_ORDER.md, possibly a feedback-policy migration after Owner decision), `08_logs/ver2_b_rls_verification.md`, `07_docs/client_feedback_policy_decision.md`. Product code: none expected; any RLS bug fix = additive idempotent migration only.
...
- **Owner checkpoint:** (1) approval to create the staging project and apply migrations **before starting**; (2) sign-off on the feedback-policy decision; (3) review of the RLS matrix. **Gate to V2-C.**

### V2-C — PC2 n8n/Modules Callback Dry-Run Integration
...
- **Deliverables:**
  ...
  - Runbook: `08_logs/ver2_c_dryrun_runbook.md` — how to run, how to verify, how to kill.
...
- **Owner checkpoint:** design doc approved **before any code**; demo of dry-run ping + kill switch at package end. **Gate to V2-F planning having any teeth.**

### V2-D — Client Demo Package
...
- **Parallel-safe with V2-B.**

### V2-E — UI/Brand Polish
...
- **Owner checkpoint:** none required to start (low risk); review at end. Can fill idle time between other work packages.

### V2-F — Real Connector Readiness Plan (plan only inside Ver2)
- **Goal:** a written, gated activation framework — so that when the Owner says "go", the path is already safe. **This work package produces documents, not connections.**
- **Deliverables:**
  ...
  - Activation order recommendation: n8n (backbone, V2-C already dry-ran it) → content tools (Canva) → analytics read-only (Meta/Google insights) → anything that *writes* to the outside world last.
- **Files likely touched:** `07_docs/` only (+ updates to `future_real_connectors.md`).
- **Safety rules:** no code, no env vars, no API registrations in this work package.
...
```
V2-A (E2E + script)  ──►  V2-B (Supabase staging) ──►  V2-C (n8n dry-run) ──► V2-F (connector plan sign-off)
        │                       │
        └──►  V2-D (demo package, parallel with V2-B)
V2-E (UI polish) — parallel filler anytime after V2-A
```

Every work package: small commits → build + test green → Codex review → Owner checkpoint → close in CURRENT_PHASE.md + logs (same discipline as MVP Phases 16A–18).

## 5. Standing safety rules for ALL of Ver2

- No real ads, posting, messaging, or live automation — at any point, in any work package.
- No secrets in the repo, ever; backend-only secret handling; `.env.local` gitignored.
- Production Supabase env stays OFF until a dedicated, Owner-approved production work package (not in this roadmap).
- Phase 16D UUID gating, tenant scoping, sanitizers, and RLS validation are load-bearing — no work package may weaken them; any change to them requires its own Codex review.
- FnB OS V1 stays untouched.
- Anything not listed in a work package's deliverables is out of that package's scope.

---

**Recommended first Ver2 implementation work package: V2-A — Manual Browser E2E Pass + Demo Script Verification.** Zero risk, no approvals needed to start, closes the MVP's only open verification gap, and gates everything else.
