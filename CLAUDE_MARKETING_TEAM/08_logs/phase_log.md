...
### 🗓️ Ngày 24/06/2026 (Local Time) — Phase R: Manual Publishing Checklist & Delivery Readiness
- **Sự kiện:** Lớp **readiness** read-only đặt TRÊN Phase Q Campaign Pack — giúp Owner/team THẤY pack của MỘT campaign đã sẵn sàng để **đăng thủ công** hay chưa, **KHÔNG thực hiện bất kỳ hành động đăng nào**. Manual-publishing readiness only, **không phải publish thật**.
- **Thiết kế:** new pure `src/lib/core/manualPublishingChecklist.ts` — `buildManualPublishingChecklist()` dựng checklist từ chính data shape Phase Q (`CampaignPackContext` + `CampaignPackItem[]` + optional `approvalRequests`); **8 section** (Owner Approval · Approved ≠ Published Safety · Copy & Captions · Creative Assets / Design Briefs · Channel Formatting · Manual Publishing Prep · Client Handoff · Metrics & Claims Disclaimer); item shape `id/label/description/status(ready|needs_owner_review|blocked|manual_action_required)/severity/owner/source_label/action_hint`; campaign-level `summary` (counts + `overall_status` ready_for_manual_publishing|needs_review_before_manual_publishing|blocked + `safety_notice` Approved ≠ Published verbatim); `renderManualPublishingChecklistText()` cho CTA copy. New read-only `src/components/core/ManualPublishingChecklistPanel.tsx` (resolve y data Phase Q, banner Approved ≠ Published + "No auto-posting. Owner/team must publish manually.", CTA **chỉ** "Copy manual checklist" + "Mark ready for manual handoff" = local note, **KHÔNG có Publish button thật**). `CampaignWorkspace.tsx` render panel sau CampaignPackPanel (giữ stateless, không đổi `App.tsx`).
- **An toàn:** approval-first (chỉ đọc item `approved`, không đổi approval state); **Approved ≠ Published** verbatim (readiness trần ở ready_for_manual_publishing, không có state published/launched); no auto-post / no auto-ads / no spend (không có path posting/schedule/launch; "Mark ready" chỉ là local note); no fake metrics (luôn có disclaimer; report ⇒ bước verify nhãn Provided/Simulated/Missing/Owner input required); no live connector / no webhook / no fetch/axios / no secrets / no env change; no Forme/sofa/furniture/nội thất/Fal.ai/ImgBB contamination.
- **Build/Test:** `npm run build` PASS (no >500 kB warning, 0 TS errors; entry `index` 376.11 kB; `CampaignWorkspace` chunk 55.42 kB). `npm run test` PASS **32 files / 342** (+2 files / +26). Log: `08_logs/phase_r_manual_publishing_checklist_delivery_readiness_20260624.md`.
- **Trạng thái:** ✅ DONE / PASS. Approval semantics: unchanged.

### 🗓️ Ngày 24/06/2026 (Local Time) — Phase S: Client Delivery Room / Shareable Handoff View
- **Sự kiện:** Preview **delivery room** read-only đặt TRÊN Phase Q + Phase R — **compose** Campaign Pack (Q) + Manual Publishing Checklist (R) thành 1 surface handoff client-facing sạch để Owner **copy / chia sẻ thủ công**. Local/read-only client handoff view only — **KHÔNG publish, KHÔNG tạo public/share URL**.
- **Thiết kế:** new pure `src/lib/core/clientDeliveryRoom.ts` — `buildClientDeliveryRoom({context, items, checklist, approvalRequests?})` trả room object ổn định (title, client/campaign summary, brand snapshot internal/draft-only, included handoff sections theo module, approval status, **readiness** [not_client_ready | delivery_not_ready | ready_for_manual_client_handoff], manual next steps từ checklist R, safety_warnings, **`published: false`** structural, explicit `approved_not_published_message` + `manual_publishing_only_message`); readiness = compose (0 approved ⇒ not_client_ready; approved + R rollup ready ⇒ ready_for_manual_client_handoff; approved nhưng blocked/needs-review ⇒ delivery_not_ready) — Approved **không bao giờ** render thành Published; `renderClientDeliveryRoomText` copyable, **không phát URL/link**. Reuse Phase Q `campaignModuleBreakdown` + handoffPack ordering + Phase R summary/`OVERALL_STATUS_LABEL` (no duplication). New read-only `src/components/core/ClientDeliveryRoomPanel.tsx` (banner Approved ≠ Published + Not Published + "No public link is created — copy and share this handoff manually."; CTA local-only **Copy client handoff summary** / **Copy manual publishing checklist** (reuse R `renderManualPublishingChecklistText`) / **Preview delivery room** toggle; **KHÔNG có Publish button**, không share/public URL). `CampaignWorkspace.tsx` render panel sau ManualPublishingChecklistPanel (giữ stateless, không đổi `App.tsx`).
- **An toàn:** approval-first (chỉ đọc item `approved`, không đổi approval state); **Approved ≠ Published** (room `published:false`, luôn render "Not Published", trần ở ready_for_manual_client_handoff); no auto-post/auto-ads/spend/schedule/launch (không có path; copy + preview local); **no public/share URL** (test assert room object + rendered text không chứa http(s)/www/webhook/share_url/public_url/token); no fake metrics (giữ nhãn Provided/Simulated/Missing/Owner input required + standing warning); no live connector / no webhook / no fetch/axios / no secrets / no env change; no Forme/sofa/furniture/nội thất/Fal.ai/ImgBB contamination.
- **Build/Test:** `npm run build` PASS (no >500 kB warning, 0 TS errors; entry `index` 376.11 kB). `npm run test` PASS **34 files / 361** (+2 files / +19). Log: `08_logs/phase_s_client_delivery_room_handoff_view_20260624.md`.
- **Trạng thái:** ✅ DONE / PASS. Approval semantics: unchanged.

### 🗓️ Ngày 25/06/2026 (Local Time) — Phase T: Client Feedback Intake & Delivery Acceptance Room
- **Sự kiện:** Mở rộng Phase S Client Delivery Room từ **handoff preview read-only** thành **workflow review client local/mock an toàn** — Owner/team xem TRƯỚC cách client feedback + delivery acceptance SẼ được ghi nhận, **KHÔNG gửi ra ngoài, KHÔNG tạo public/share URL thật, KHÔNG email/notification/connector**. Đây **KHÔNG** phải client portal thật.
- **Thiết kế:** new pure `src/lib/core/deliveryAcceptance.ts` — (a) **mock feedback model**: `DeliveryFeedbackType` (general_comment · revision_request · approval_note · publishing_question) × `DeliveryFeedbackStatus` (open · acknowledged · resolved); `DeliveryFeedbackEntry` có `message` + **sample** `author_label` (luôn là sample, KHÔNG yêu cầu định danh client thật) + `createdAt/updatedAt` an toàn; pure mutators `addDeliveryFeedback`/`setDeliveryFeedbackStatus`/`removeDeliveryFeedback`/`listDeliveryFeedback` (mỗi cái trả NEW array, không mutate input) + `canTransitionFeedback`. (b) **acceptance state model**: `draft_preview` → `shared_for_review_mock` → `client_feedback_open` → `revision_needed` → `client_accepted` → `owner_ready_for_manual_publish` (fixed graph `DELIVERY_ACCEPTANCE_TRANSITIONS`); `transitionAcceptance(from,to,{publishingOverall})` validate graph **+ owner-ready GATE**: tới `owner_ready_for_manual_publish` bắt buộc `from==='client_accepted'` **VÀ** Phase R rollup `=== 'ready_for_manual_publishing'` (`isPublishingChecklistReady`); **KHÔNG có state `published`/`launched`** trong model. (c) `buildDeliveryAcceptanceRoom(...)` trả room ổn định (state label/desc, feedback newest-first + summary + `has_open_revision_request`, `publishing_checklist_ready`, `can_mark_owner_ready_for_manual_publish` + `owner_ready_blocked_reason`, `next_actions`, `local_only_badges`, **`published:false`** structural, not-published + mock-note messages); `renderDeliveryAcceptanceText` copyable, **không phát URL/link**. Verbatim safety: `DELIVERY_ACCEPTANCE_LOCAL_ONLY_BADGES` (Local preview only · No public URL created · No notification sent · No connector used), `DELIVERY_ACCEPTANCE_NOT_PUBLISHED` ("Client accepted ≠ Published. Publishing remains manual and owner-controlled."), `DELIVERY_ACCEPTANCE_MOCK_NOTE`, `DELIVERY_ACCEPTANCE_OWNER_READY_REQUIRES_CHECKLIST`; deterministic `sampleDeliveryFeedback(now?)` (FnB sample, author `(sample)`). New self-contained `src/components/core/DeliveryAcceptancePanel.tsx` (giữ feedback + acceptance state trong local React state seed từ `sampleDeliveryFeedback`; resolve cùng context + APPROVED deliverables Phase Q/R/S → build Phase R checklist chỉ để đọc `overall_status` gate owner-ready; Client Feedback panel mock intake [add/list/acknowledge/resolve/reopen/remove]; Delivery Acceptance ladder + transition buttons, nút owner-ready **disabled kèm lý do** tới khi client_accepted + checklist ready; badges local/mock + Not Published rõ ràng; banner "Client accepted ≠ Published…"; CTA chỉ **Copy acceptance summary** [clipboard, gate `can.exportPacks`] + **Preview** [textarea read-only]; **KHÔNG có Publish/Send/Notify button**, không share/public URL, không network). `CampaignWorkspace.tsx` render panel sau ClientDeliveryRoomPanel (giữ stateless, không đổi `App.tsx`).
- **An toàn:** approval-first (ghi feedback/đổi acceptance state KHÔNG mutate approval state; chỉ đọc item `approved` để dựng gate); **Approved ≠ Published** (room `published:false`, `client_accepted` KHÔNG phải published, không state published/launched, `owner_ready_for_manual_publish` terminal + gated sau Phase R checklist = bước publish thủ công Owner-controlled bên ngoài module); **local/mock only** (no email, **no public/share URL**, no notification, no connector, no sync — test assert cả rendered text + panel source); no auto-post/auto-ads/spend/auto-approve (CTA chỉ copy + preview; source scan ban affirmative publish/launch/run-ads/share-url + auto-approve; auto-post chỉ negated); no secrets / no live connector / no webhook / no fetch/axios / no env change / no fake metrics / no live analytics; no Forme/sofa/furniture/nội thất/Fal.ai/ImgBB contamination.
- **Build/Test:** `npm run build` PASS (no >500 kB warning, 0 TS errors; entry `index` 376.11 kB; `CampaignWorkspace` chunk 89.54 kB). `npm run test` PASS **36 files / 400** (+2 files / +39). Log: `08_logs/phase_t_client_feedback_intake_delivery_acceptance_20260625.md`.
- **Trạng thái:** ✅ DONE / PASS. Approval semantics: unchanged.

### 🗓️ Ngày 25/06/2026 (Local Time) — Phase U: Delivery Closure & Manual Publishing Handoff Control
- **Sự kiện:** Lớp **closeout local/demo** đặt TRÊN Phase T (acceptance) + Phase R (manual publishing checklist) — sau khi client accepted, làm rõ tình huống sau-acceptance: Owner/team THẤY closure status, unresolved feedback, manual-publishing readiness, external publishing owner, closure notes, audit trail local — và đóng delivery **KHÔNG ngụ ý connector thật hay auto publishing**. **Client accepted ≠ Published**, publishing thực hiện **thủ công ngoài CORE**, "manually marked as published" chỉ là **annotation thủ công của operator** (không bao giờ auto-set).
- **Thiết kế:** new pure `src/lib/core/deliveryClosure.ts` — (a) **closure status model** `DeliveryClosureStatus` (not_accepted · client_accepted_not_published · ready_for_manual_publishing · manually_marked_published · closed_without_publishing) + label/color/description; **KHÔNG có state `published`/`launched` do CORE set**. (b) `deriveClosureStatus(input)` thuần từ input tường minh (acceptance state, Phase R `publishingOverall`, unresolved feedback count, `feedbackCarriedForward`, `manualPublishMark`); terminal states **chỉ** đạt được qua `ManualPublishMark` tường minh (marked_published / closed_unpublished), **không bao giờ suy ra**; chưa mark thì accepted = client_accepted_not_published → ready_for_manual_publishing khi Phase R ready + feedback clear (**vẫn chưa published**). (c) `buildDeliveryClosure(params)` compose status + acceptance + feedback (unresolved/resolved + `unresolved_feedback_warning`) + Phase R readiness + **6-item manual closure checklist** (client_acceptance_reviewed · final_pack_reviewed · feedback_resolved_or_carried · manual_publishing_checklist_reviewed · external_publishing_owner_assigned · external_publishing_status_marked) + `checklist_complete` + `ready_to_close` + manual mark + external owner + notes; luôn **`core_published:false`**. (d) **audit** pure (`ClosureAuditEventType` ×5, `appendClosureAudit` trả NEW array, mỗi entry `local_mock:true`, `sampleClosureAudit`). Verbatim safety: `CLOSURE_CLIENT_ACCEPTED_NOT_PUBLISHED` ("Client accepted ≠ Published."), `CLOSURE_PUBLISH_OUTSIDE_CORE` ("Publishing must be completed manually outside CORE."), `CLOSURE_SAFETY_NOTE`, `CLOSURE_NO_AUTOMATION_NOTE`, `CLOSURE_LOCAL_ONLY_BADGES`; `renderDeliveryClosureText` copyable, **không phát URL/link**, in "Not published by CORE". New self-contained `src/components/core/DeliveryClosurePanel.tsx` (local React state: acceptance selector, checklist confirms, carry-forward toggle, manual mark, external owner, notes, audit seed `sampleClosureAudit`; resolve cùng context + APPROVED deliverables Phase Q/R/S/T → Phase R `overall_status`; status callout + acceptance summary + unresolved-feedback warning + carry-forward; 6 checklist gates + external owner input; handoff controls record-acceptance/record-checklist/record-ready/**Mark published manually (outside CORE)** [gated `ready_to_close`]/**Close without publishing**/reset-mark, mỗi nút append audit local; badges local/demo + Not Published; banner "Client accepted ≠ Published." + "Publishing must be completed manually outside CORE."; CTA chỉ **Copy closure summary** [clipboard, gate `can.exportPacks`] + **Preview**; **KHÔNG có Publish/Send/Notify button**, không share/public URL, không network). `CampaignWorkspace.tsx` render panel sau DeliveryAcceptancePanel (giữ stateless, không đổi `App.tsx`).
- **An toàn:** approval-first (ghi closure / mark publish status KHÔNG mutate approval state; chỉ đọc item `approved` dựng Phase R readiness); **Client accepted ≠ Published** (structural `core_published:false`, copy verbatim luôn hiển thị, client_accepted_not_published & ready_for_manual_publishing đều non-published, không state published/launched do CORE set, `manually_marked_published` là annotation thủ công outside CORE **không bao giờ auto-derive** + gated sau closure checklist); **local/demo only** (no email, **no public/share URL**, no notification, no connector, no sync; audit gắn local/mock/demo — test assert cả rendered text + panel source); no auto-post/auto-ads/spend/auto-approve (CTA chỉ copy + preview; source scan ban affirmative publish/launch/run-ads/share-url + auto-approve; auto-post/auto-publish/auto-launch chỉ negated); no secrets / no live connector / no webhook / no fetch/axios / no env change / no fake metrics / no live analytics; no Forme/sofa/furniture/nội thất/Fal.ai/ImgBB contamination (token `perform*` được tránh để né false-positive `Forme`).
- **Codex fix round (2026-06-25):** `deriveClosureStatus()` trước đây trả `manually_marked_published`/`closed_without_publishing` TRƯỚC khi kiểm tra điều kiện safe-close → một manual mark có thể ép terminal state lên delivery chưa accepted / feedback chưa xử lý / Phase R chưa ready / checklist chưa đủ (UNSAFE). Fix: thêm guard thuần dùng chung **`hasSafeClosureReadiness(input)`** (accepted ∈ {client_accepted, owner_ready_for_manual_publish} + feedback resolved/carried-forward + Phase R `ready_for_manual_publishing` + `closureGatesComplete` [mọi gate trừ chính status-mark, loại để tránh circular]) + wrappers `canMarkManualPublished`/`canCloseDelivery`; `deriveClosureStatus` chỉ honor mark khi guard đúng, mark unsafe bị bỏ qua → fall through về status an toàn (not_accepted/client_accepted_not_published/ready_for_manual_publishing), KHÔNG tạo status mới; `buildDeliveryClosure` build checklist trước → tính `closure_gates_complete` → derive status → báo **effective mark** (`manual_publish_mark` = mark yêu cầu CHỈ khi đã áp dụng `manual_publish_mark_applied`, else `none`) + `safe_closure_ready` + `manual_publish_mark_requested`. Panel: `handleCloseUnpublished` early-return + nút "Close without publishing" disabled tới khi `ready_to_close` (đối xứng với Mark published); cảnh báo đỏ khi mark requested-but-not-applied. Tests +11 (marked_published & closed_unpublished đều bị ignore khi not-accepted/feedback-unresolved/Phase-R-not-ready/gates-incomplete; positive chỉ honor khi đủ điều kiện; guard predicates; end-to-end buildDeliveryClosure; cập nhật 2 test cũ theo behavior an toàn mới + 1 panel source-scan assert cả 2 mark handler gated).
- **Build/Test:** `npm run build` PASS (no >500 kB warning, 0 TS errors; entry `index` 376.11 kB; `CampaignWorkspace` chunk 114.17 kB). `npm run test` PASS **38 files / 447** (+2 files / +47; +11 từ Codex fix round). Log: `08_logs/phase_u_delivery_closure_manual_publishing_handoff_control_20260625.md`.
- **Trạng thái:** ✅ DONE / PASS (chờ Codex re-review). Approval semantics: unchanged.

### 🗓️ Ngày 25/06/2026 (Local Time) — Phase V: Manual Publishing Evidence & Result Intake Room
- **Sự kiện:** Lớp **evidence post-delivery local/demo** đặt TRÊN Phase U (closure) + Phase T (acceptance) — sau khi delivery accepted/closed, cho Owner **ghi nhận thủ công** asset đã thực sự được đăng **ngoài CORE** hay chưa, **KHÔNG publish, KHÔNG ads, KHÔNG connector/endpoint thật, KHÔNG live analytics, KHÔNG tạo link**. **CORE không publish** — mọi giá trị là **evidence Owner-provided** (hoặc client-provided / simulated demo). **Approved ≠ Published** và **Client Accepted ≠ Published** giữ tường minh; **"published" chỉ là bản ghi thủ công của Owner**; `scheduled_outside_core` **KHÔNG phải published**; metrics **không bao giờ trình bày như thật** trừ khi owner/client-provided.
- **Thiết kế:** new pure `src/lib/core/manualPublishingEvidence.ts` — (a) **publish status model** `PublishStatus` (not_published · manually_published · scheduled_outside_core · blocked_or_cancelled) + label/color/description; **KHÔNG có state `published`/`launched` do CORE set**; predicates `isManuallyPublished`/`isScheduledNotPublished`/`isPublishedRecord` (chỉ `manually_published` mới tính published; scheduled KHÔNG). (b) **result source + metrics** `ResultDataSource` (not_provided · owner_provided · client_provided · simulated_demo) × `ResultMetricKey` (reach · impressions · engagement · messages · leads · orders · revenue, optional manual-only); `metricsRealClaimAllowed` chỉ true cho owner/client-provided; `metricsPresentation` → none|manual_provided_real|simulated_or_unverified. (c) **evidence record** `ManualPublishingEvidence` (campaignId, contentItemId?, channel, publishStatus, publishedAt?/manualScheduledAt?/publishedBy?/publicUrl?/evidenceNote?, resultDataSource, metrics?, notes?, **`local_mock:true`**) + pure mutators `createEvidence`/`addEvidence`/`updateEvidence`/`setEvidenceStatus`/`removeEvidence`/`listEvidence` (mỗi cái trả NEW array) + `cleanMetrics`; `publicUrl` là **dữ liệu Owner dán, KHÔNG phải link CORE tạo**. (d) `validateEvidence` — **manually_published bắt buộc channel + publishedBy + (evidence note HOẶC publicUrl)** (errors chặn); **scheduled_outside_core cảnh báo KHÔNG phải published**; **metrics simulated/not-provided cảnh báo KHÔNG được trình bày như thật**. (e) `buildPublishingEvidenceReport` — tóm tắt CHỈ những gì ghi tay (count theo status, metric cells + nhãn provenance, missing flags, notices), **`core_published:false`**. Verbatim safety: `EVIDENCE_CORE_DOES_NOT_PUBLISH` ("CORE does not publish. This is Owner-provided evidence only."), `EVIDENCE_NO_LIVE_ANALYTICS` ("No live analytics connected."), `EVIDENCE_MANUAL_ONLY` ("Manual evidence only."), `EVIDENCE_APPROVED_NOT_PUBLISHED`, `EVIDENCE_CLIENT_ACCEPTED_NOT_PUBLISHED`, `EVIDENCE_PUBLISHED_MEANS_MANUAL`; `renderPublishingEvidenceReportText` copyable, in metric simulated rõ **SIMULATED / UNVERIFIED, not real**, **không phát link CORE tạo**; deterministic `sampleManualPublishingEvidence` (FnB Vị Cuốn sample, publicUrl scheme-less `(sample)`). New self-contained `src/components/core/ManualPublishingEvidencePanel.tsx` (evidence list trong local React state seed từ sample; **4 surface bắt buộc**: Publishing status summary + Add row · Evidence/URL/screenshot note (publicUrl nhãn *Owner-provided evidence*) · Manual result intake (source select + 7 metric inputs, badge *May show as provided data* vs *Simulated/unverified — not real*) · Post-Publish Report Draft (provenance + missing flags + notices); per-row validation errors/warnings inline; badges local/demo + Not Published by CORE; banner "CORE does not publish…" + "No live analytics connected." + "Manual evidence only." + "Approved ≠ Published. Client Accepted ≠ Published."; CTA chỉ **Copy evidence report** [clipboard, gate `can.exportPacks`] + **Preview**; **KHÔNG có Publish/Send/Notify/Launch button**, không share/public URL do CORE tạo, không network). `CampaignWorkspace.tsx` render panel sau DeliveryClosurePanel (giữ stateless, không đổi `App.tsx`).
- **An toàn:** approval-first (ghi evidence / đổi publish status KHÔNG mutate approval state; panel không chạm approval data); **Approved ≠ Published & Client Accepted ≠ Published** (structural `core_published:false`, copy verbatim luôn hiển thị, không state published/launched do CORE set, `manually_published` là annotation Owner outside CORE **không bao giờ auto-derive**, `scheduled_outside_core` KHÔNG phải published); **CORE không publish** (no publishing/posting/scheduling/launching/spend; publicUrl là dữ liệu Owner-pasted); **no fake metrics / no live analytics** (metrics manual-only, real chỉ khi owner/client-provided, còn lại nhãn simulated/unverified; report luôn "No live analytics connected."/"Manual evidence only." + flag missing); no auto-post/auto-ads/spend/auto-approve (CTA chỉ copy + preview; source scan ban affirmative publish/launch/run-ads/share-url + auto-approve; auto-post/auto-ads chỉ negated); no secrets / no live connector / no real webhook/endpoint / no fetch/axios / no env change; no Forme/sofa/furniture/nội thất/Fal.ai/ImgBB contamination (token `webhook`/`OAuth`/`perform*` tránh trong production source).
- **Build/Test:** `npm run build` PASS (no >500 kB warning, 0 TS errors; entry `index` 376.11 kB; `CampaignWorkspace` chunk 137.87 kB). `npm run test` PASS **40 files / 485** (+2 files / +38). Log: `08_logs/phase_v_manual_publishing_evidence_result_intake_20260625.md`.
- **Trạng thái:** ✅ DONE / PASS (chờ Codex review). Approval semantics: unchanged.

### 🗓️ Ngày 29/06/2026 (Local Time) — Phase W: Manual Publishing Result Review & Campaign Learning Loop
- **Sự kiện:** Lớp **review deterministic local/demo** đặt TRÊN Phase V (evidence) — **REVIEW (không thu thập)** dữ liệu kết quả Owner ghi tay rồi dựng result summary + evidence/attribution/confidence quality + risk flags + repeat/avoid recommendations + next actions + **Brand Brain LEARNING CANDIDATE PREVIEW**. **KHÔNG live analytics, KHÔNG connector, KHÔNG AI/API/network, KHÔNG đổi Published status, KHÔNG ghi/auto-update Brand Brain.** **Published chỉ là bản ghi thủ công của Owner** (reuse `isPublishedRecord`); learning là **candidate/preview only**.
- **Thiết kế:** new pure `src/lib/core/manualResultReview.ts` — reuse Phase V types/predicates (`ManualPublishingEvidence`, `ResultMetrics`/`ResultDataSource`, `isPublishedRecord`, `metricsRealClaimAllowed`, `metricsPresentation`, các safety constants) làm single source of truth; **review status** `no_manual_evidence` · `evidence_logged_result_pending` · `provided_manual_result_reviewed`; output đầy đủ (reviewStatus, resultSummary chỉ in giá trị **provided** + nhãn missing, performanceSignal bảo thủ "based on provided manual data", confidenceLevel/evidenceQuality/attributionQuality, riskFlags [incomplete_conversion_data · weak_attribution · customer_complaint · stockout_or_capacity_issue · content_accuracy_issue · timing_issue · unverified_metrics], learningCandidates, repeat/avoidRecommendations, nextActionSuggestions, **brandBrainLearningCandidatePreview** `persisted_to_brand_brain:false`/`is_brand_brain_update:false`/`sufficiency`, structural `core_published:false`/`published_semantics_unchanged:true`/`persisted_to_brand_brain:false`); rule deterministic (spend Owner-provided optional thiếu conversion → incomplete; orders/revenue thiếu evidence/real source → weak_attribution; note scan EN+VN chê/hết hàng/sai giá/đăng muộn → flag; entry tín hiệu mạnh nhất → repeat angle "Based on provided manual data"; owner-estimate-only cap confidence dưới high; simulated/not-provided → unverified, không bao giờ coi là thật); **KHÔNG import `brandBrain.ts`** (decoupled). New read-only `src/components/core/ManualResultReviewPanel.tsx` (consume **`evidence` prop**, KHÔNG seed sample riêng; 10 section bắt buộc; banner "Manual review only … Reviews only the manual publishing evidence recorded above — nothing is invented … No live analytics … Does not change Published status … Learning candidate only — Not persisted to Brand Brain yet … Approved ≠ Published. Client Accepted ≠ Published."; CTA chỉ Copy/Preview, không network). New stateful wrapper `src/components/core/ManualPublishingEvidenceSection.tsx` — sở hữu **một** shared evidence state **default EMPTY**, render Phase V panel (controlled) + Phase W review (cùng `evidence`).
- **Codex fix round (2026-06-29):** (1) *Panels không nối state* → Phase V `ManualPublishingEvidencePanel` chuyển **controlled** (`evidence`+`onChange`), state nâng lên `ManualPublishingEvidenceSection` (CampaignWorkspace **vẫn stateless** theo Phase K guard — `useState` nằm ở wrapper); review giờ review **đúng** evidence Owner ghi ở Phase V. (2) *Sample/Vị Cuốn bị render như kết quả thật* → shared state **default EMPTY** ⇒ mặc định review = `no_manual_evidence` ("cannot review"), KHÔNG ngụ ý có post Vị Cuốn đã published/reviewed; sample Phase V thành **opt-in "Load example data (sample)"** rõ nhãn, KHÔNG auto-load; review panel **không** dùng `sampleManualResultEntries`/`sampleManualPublishingEvidence`. (3) *Thiếu Phase W log* → file này + `08_logs/phase_w_manual_result_review_learning_loop_20260629.md`.
- **An toàn:** approval-first (review/feedback KHÔNG mutate approval state); **Approved ≠ Published & Client Accepted ≠ Published** verbatim; **Published chỉ = bản ghi Owner thủ công** (reuse `isPublishedRecord`, `core_published:false`, không set published/launched); **Brand Brain không auto-update** (`persisted_to_brand_brain:false`, module không import brandBrain); learning **candidate/preview only**; **no live analytics / no fake metrics** (chỉ echo provided, simulated→unverified); no auto-post/auto-ads/auto-publish (chỉ negated trong source scan); no secrets / no live connector / no webhook / no fetch/axios / no OAuth / no env change; no Forme/sofa/furniture/nội thất/Fal.ai/ImgBB contamination.
- **Build/Test:** `npm run build` PASS (no >500 kB warning, 0 TS errors; entry `index` 376.11 kB; `CampaignWorkspace` chunk ~163.87 kB). `npm run test` PASS **42 files / 525** (+2 files / +40 từ Phase V baseline). Log: `08_logs/phase_w_manual_result_review_learning_loop_20260629.md`.
- **Trạng thái:** ✅ Implemented locally + validated · 🟡 **PENDING FINAL CODEX APPROVAL** · **chưa commit / chưa push**. Approval semantics: unchanged.

### 🗓️ Ngày 01/07/2026 (Local Time) — Phase X: Owner-Approved Campaign Learning Memory / Brand Brain Learning Candidate Approval
- **Sự kiện:** Biến **learning candidates** của Phase W thành **luồng duyệt Owner accept/reject** rồi dựng **Brand Brain update proposal** CHỈ từ candidate được chấp nhận. **Owner là quyền duyệt DUY NHẤT** (owner-only gate, KHÔNG phải manager). **Accepted ≠ đã ghi vào Brand Brain**; **Brand Brain source of truth KHÔNG bao giờ bị mutate/auto-update** (không có mutator, và không thêm mutator); rejected/pending **không được dùng**; **Approved ≠ Published & Client Accepted ≠ Published**; KHÔNG AI/API, KHÔNG connector, KHÔNG live analytics, KHÔNG auto-post/auto-ads, KHÔNG fake metrics, KHÔNG secrets/env/OAuth/webhook/fetch/axios/network.
- **Thiết kế:** new pure `src/lib/core/brandBrainLearning.ts` — `LearningDecision` (`pending`/`accepted`/`rejected`), `candidateKey` (id ổn định theo nội dung candidate ⇒ quyết định của Owner sống sót khi Phase W re-derive candidate từ evidence đã sửa), `initLearningReviews` (carry-forward quyết định cũ theo id), `applyLearningDecision` (accept/reject/reset → trả **NEW** `{reviews, audit}` array + audit entry bất biến; no-op nếu id lạ / decision không hợp lệ; `pending` sau khi đã quyết → clear metadata, audit là `reset`), `summarizeLearningReviews`, `listLearningAudit`, `buildBrandBrainUpdateProposal` (**accepted-only, prepared-only**: `persisted_to_brand_brain:false` / `is_applied_to_brand_brain:false` / `requires_owner_approval_to_apply:true`), `renderBrandBrainUpdateProposalText`, verbatim safety copy. **KHÔNG import `brandBrain.ts`** (decoupled — proposal là artifact riêng, không phải `BrandBrain`). New `src/components/core/BrandBrainLearningReviewPanel.tsx` — gate `can.publishContent` (owner-only); 4 surface (decision summary · learning candidates với Accept/Reject/Reset + optional reason · prepared Brand Brain update proposal accepted-only "Applied to Brand Brain: NO" · local decision audit trail); consume **`evidence` prop**, derive candidate qua Phase W `buildManualResultReview`, **không seed sample riêng**; CTA chỉ Copy/Preview (clipboard), no network. Wire vào `ManualPublishingEvidenceSection.tsx` làm panel **thứ ba** SAU Phase W review, dùng chung một `evidence` state (CampaignWorkspace vẫn stateless).
- **An toàn:** approval-first (quyết định là hành động Owner tường minh, không auto); Owner-only (owner gate, không manager, không `isInternalRole`/`can.approveContent`); **Accepted ≠ applied/written to Brand Brain** (`is_applied_to_brand_brain:false`, proposal structural flags); **Brand Brain không mutate/auto-update** (no mutator, module decoupled); rejected/pending không được dùng (proposal accepted-only); **Approved ≠ Published & Client Accepted ≠ Published** verbatim; audit trail đầy đủ (candidate source id, action, actor, reason, timestamp); no live analytics / no fake metrics (chỉ re-label candidate Phase W, không bịa số); no auto-post/auto-ads/auto-publish (chỉ negated trong source scan); no secrets / no connector / no webhook / no fetch/axios / no OAuth / no env / no localStorage; no Forme/sofa/furniture/nội thất/Fal.ai/ImgBB/Vị Cuốn contamination.
- **Build/Test:** `npm run build` PASS (no >500 kB warning, 0 TS errors; `CampaignWorkspace` chunk ~178.52 kB). `npm run test` PASS **44 files / 560** (+2 files / +35 từ Phase W baseline: 20 domain + 15 panel-source). Log: `08_logs/phase_x_owner_approved_brand_brain_learning_candidate_approval_20260701.md`.
- **Trạng thái:** ✅ Implemented locally + validated · 🟡 **PENDING CODEX REVIEW** · **chưa commit / chưa push**. Approval semantics: unchanged.
